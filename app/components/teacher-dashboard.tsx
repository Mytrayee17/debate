...
function GamificationSummary() {
  const { points, level, badges, progress } = useGamificationStore();
  return (
    <Card className="mb-6 border-blue-400 bg-blue-50">
      ...
      <CardContent>
        <div className="flex flex-col md:flex-row md:items-center md:space-x-8 space-y-4 md:space-y-0">
          <div>
            <div className="text-lg font-bold">Points: <span className="text-blue-700">{points}</span></div>
            <div className="text-lg font-bold">Level: <span className="text-blue-700">{level}</span></div>
            <div className="mt-2 flex flex-wrap gap-2">
              {badges.length > 0 ? badges.map((badge) => (
                <Badge key={badge} className="bg-yellow-200 text-yellow-800 border-yellow-400">{badge}</Badge>
              )) : <span className="text-gray-500">No badges yet</span>}
            </div>
          </div>
          <div className="flex-1">
            <div className="text-sm text-gray-600 mb-1">Progress Over Time</div>
            <div className="w-full h-16 bg-white rounded shadow flex items-end overflow-x-auto">
              {progress.length > 1 ? progress.map((entry, i) => (
                <div key={i} className="flex flex-col items-center mx-1">
                  <div className="w-2 bg-blue-400 rounded" style={{ height: `${Math.max(8, entry.points / 10)}px` }} />
                  <span className="text-xs text-gray-400 mt-1">{new Date(entry.date).toLocaleDateString()}</span>
                </div>
              )) : <span className="text-gray-400 ml-2">Complete modules to see progress</span>}
            </div>
          </div>
        </div>
      </CardContent>
...
${selectedClass?.students
  ...
  return (
    <div className="space-y-6">
      {/* Gamification summary for the current user */}
      <GamificationSummary />
      {/* Header */}
